...
- **监控功能**：WebSocket 长连接静默上报切屏与退出全屏行为，监控过程考生无感知；首次切屏即触发警告遮罩
- **强制收卷**：管理员远程终止考试，代码自动保存
- **多题型**：编程题、选择题（单选 / 多选）、填空题三种题型，题目内容以 Markdown 存储与渲染
- **操作日志**：关键行为留痕，按 normal / warning / critical 分级
- **软删除**：考试、题目、考生逻辑删除，数据不物理移除
- **数据导出**：考试数据（代码、日志、成绩）一键导出，zip 内含 `export_info.txt` 记录格式版本号，下游工具按版本解析
...
后端容器启动时自动执行数据库迁移（首次部署建表、升级时更新结构），无需手动操作。手动兜底命令（直调镜像内可执行文件，不经过 uv，零外网依赖）：
